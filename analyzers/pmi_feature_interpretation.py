...
class FeatureInterpreter:
    ...
    def print_summary(results: Dict[int, FeatureInterpretation]) -> None:
        ...
        sep = "=" * 110
        print(f"\n{sep}")
        print(f"{'IDX':>6}  {'CONF':^8}  {'ACT RATE':>8}  {'DOM POS':>8}  "
              f"{'PMI TOKENS':<30}  INTERPRETATION")
        print(sep)
        for feat_idx in sorted(results.keys()):
            r = results[feat_idx]
            toks = ", ".join(r.summary_tokens[:4]) if r.summary_tokens else "-"
            # Truncate interpretation to fit terminal width
            interp = r.interpretation
            # Each pipe-delimited segment is one analysis — show them on separate lines
            segments = [s.strip() for s in interp.split(" | ")]
            first_line = segments[0] if segments else "-"
            print(
                f"{feat_idx:>6}  {r.confidence:^8}  "
                f"{r.activation_rate:>8.2%}  {r.dominant_pos:>8}  "
                f"{toks:<30}  {first_line}"
            )
            for seg in segments[1:]:
                print(f"{'':>6}  {'':^8}  {'':>8}  {'':>8}  {'':30}  {seg}")
        print(sep)
