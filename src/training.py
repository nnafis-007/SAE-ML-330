...
# Convenience alias so type hints stay readable throughout this file.
SAEModel = Union[SparseAutoencoder, TopKSparseAutoencoder]
...
class SAETrainer:
    ...
    def __init__(
        ...
        # Store grad_clip_norm so train_epoch can respect the configured value.
        # A value of 0.0 disables gradient clipping entirely.
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch in tqdm(self.train_loader, desc="Training", leave=False):
            ...
            # Disabled when grad_clip_norm == 0 (pass 0.0 to SAETrainer to skip).
    ...
    def train(
        ...
        for epoch in range(num_epochs):
            ...
            if (
                ...
                freqs = self._estimate_feature_frequencies(
                    max_samples=resample_eval_samples
                )
                dead_mask = freqs < float(resample_freq_threshold)
                num_resampled = self._resample_features(
                    dead_mask, max_features=resample_max_features
                )
                if num_resampled > 0:
                    print(
    ...
    def _resample_features(
        self, dead_mask: torch.Tensor, max_features: int = 2048
    ) -> int:
        ...
        # After this point W_dec is guaranteed to be a Parameter (not None).
        # The assert narrows the type so static checkers don't flag subsequent accesses.
        assert self.model.W_dec is not None
        ...
        if batch is None or batch.numel() == 0:
            ...
            self.model.W_enc.data[dead_indices] = torch.randn(
                dead_indices.numel(), d_model, device=self.model.W_enc.device
            ) / (d_model**0.5)
            self.model.b_enc.data[dead_indices] = 0.0
            new_cols = torch.randn(
                d_model, dead_indices.numel(), device=self.model.W_dec.device
            )
            ...
            if (
                self.model.use_pre_bias
                and getattr(self.model, "b_pre", None) is not None
            ):
                x_centered = x - self.model.b_pre
            else:
                x_centered = x
            preact = torch.matmul(
                x_centered, self.model.W_enc.data[dead_indices].T
            )  # (batch, k)
    ...
    def save_checkpoint(self, filename: str, epoch: int, metrics: Dict[str, float]):
        ...
        checkpoint = {
            ...
            "hyperparameters": {
                ...
                # l1_coeff is 0.0 for TopKSparseAutoencoder; kept for compatibility.
                "l1_coeff": getattr(self.model, "l1_coeff", 0.0),
                # Top-K specific — None for standard SAE.
    ...
    def plot_training_history(self, save_path: Optional[str] = None):
        ...
        2. MSE component
        3. L1 component (flat zero for Top-K SAEs) or labelled "Top-K (no L1)"
        4. Feature density over time
        """
        fig, axes = plt.subplots(2, 2, figsize=(15, 10))

        epochs = range(1, len(self.history["train_loss"]) + 1)

        # Detect whether L1 was actually used (non-zero for at least one epoch).
        l1_used = any(v > 1e-12 for v in self.history["train_l1"])
        l1_title = (
            "Sparsity Penalty (L1)"
            if l1_used
            else "L1 Penalty (Top-K: structural sparsity)"
        )
        total_title = "Total Loss (MSE + L1)" if l1_used else "Total Loss (MSE only)"
        ...
        axes[0, 0].set_title(total_title)
        axes[0, 0].legend()
        ...
        # Plot 3: L1 Loss (or informational note for Top-K)
        ...
        axes[1, 0].set_title(l1_title)
        if not l1_used:
            axes[1, 0].text(
                0.5,
                0.5,
                "Top-K SAE: sparsity is\nenforced structurally (no L1)",
...
def train_sae(
    ...
    """
    Convenience function to create and train a standard (L1) SAE.

    For the Top-K variant use train_topk_sae() or instantiate SAETrainer
    directly with a TopKSparseAutoencoder.
    ...
    # Plot results
    trainer.plot_training_history(save_path=f"{checkpoint_dir}/training_history.png")
...
def train_topk_sae(
    ...
    device: str = "cuda" if torch.cuda.is_available() else "cpu",
    checkpoint_dir: str = "checkpoints",
) -> tuple[TopKSparseAutoencoder, Dict[str, List[float]]]:
    """
    Convenience function to create and train a Top-K SAE.
    ...
    Args:
        ...
        k:                        Number of active features per token.
                                  Auto-computed via suggest_k() when None.
        lr:                       Learning rate.
        batch_size:               Batch size.
        num_epochs:               Maximum epochs.
        early_stopping_patience:  Early stopping patience.
    ...
    Returns:
        Tuple of (trained TopKSparseAutoencoder, training history dict).
    """
    from sae_model import TopKSparseAutoencoder, suggest_k

    if k is None:
        k = suggest_k(d_model, expansion_factor=expansion_factor)
        print(
            f"Auto-selected k = {k} for d_model={d_model}, expansion={expansion_factor}×"
        )
