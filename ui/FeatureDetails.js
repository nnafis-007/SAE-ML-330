...
export default function FeatureDetails({ feature, onClose, modelId }) {
  ...
  const confidenceColor = (conf) => {
    if (conf === 'high') return C.confHigh;
    if (conf === 'medium') return C.confMed;
    return C.confLow;
  };
          ...
          {llmLabel && (
            <View style={styles.resultPanel}>
              <View style={styles.sectionLabelRow}>
                ...
                <Text style={[styles.sectionLabel, { color: C.pink }]}>LLM LABEL RESULT</Text>
          ...
          {(usePromptAlignedRows || activationPayload?.matches?.length > 0) && (
            <View style={styles.matchesPanel}>
              <View style={styles.sectionLabelRow}>
                ...
                <Text style={[styles.sectionLabel, { color: C.yellow }]}>
