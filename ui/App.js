          ...
          <View style={[styles.contentContainer, isLargeScreen ? styles.row : styles.column]}>
            ...
            <View style={[styles.panel, styles.leftPanel, isLargeScreen && styles.leftPanelLarge]}>
              <View style={styles.layerSelectorContainer}>
                <Text style={styles.layerSelectorLabel}>Pretrained SAE Model</Text>
                  ...
                  {modelsLoading ? (
                    ...
                    >
                      {models.map((m) => (
                        <Picker.Item
                          key={m.id}
                          label={`${m.id}  (${m.d_hidden || '?'} features)`}
                          value={m.id}
                        />
                      ))}
                    </Picker>
              ...
              <View style={styles.layerSelectorContainer}>
                ...
                <View style={styles.layerPickerWrapper}>
                  {modelsLoading ? (
                    ...
                    >
                      {models.map((m) => (
                        <Picker.Item key={m.id} label={`${m.id}  (${m.d_hidden || '?'} features)`} value={m.id} />
                      ))}
                    </Picker>
                  )}
                </View>
          ...
          <View style={styles.contentContainer}>
            <View style={styles.panel}>
              ...
              <View style={styles.featureLookupCard}>
                <Text style={styles.instructionText}>
                  Select any feature ID and fetch sentences (+tokens) from the configured dataset where it activates.
                </Text>
              ...
              {tokens.length > 0 && activatedTokenRows.length === 0 && (
                <View style={styles.placeholderCard}>
                  ...
                  <Text style={styles.placeholderSubText}>Try a different sentence or pretrained SAE model.</Text>
                </View>
              )}
